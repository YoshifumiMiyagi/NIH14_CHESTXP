...
class EVACXRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            torch.tensor(sx_i, dtype=torch.long),
...
@torch.no_grad()
def predict_probs_unlabeled(model, loader, device, use_amp=True):
    model.eval()
    device_type = "cuda" if str(device).startswith("cuda") else "cpu"
    amp_enabled = bool(use_amp and device_type == "cuda")

    probs_list, age_list, idx_list = [], [], []

    for x, age, idx in loader:
        x = x.to(device, non_blocking=True)
        with torch.amp.autocast(device_type, enabled=amp_enabled):
            logits = model(x)
            prob = torch.softmax(logits, dim=1)[:, 1]
    ...
    probs = np.concatenate(probs_list) if probs_list else np.array([], dtype=np.float32)
    age = np.concatenate(age_list) if age_list else np.array([], dtype=np.float32)
    idx = np.concatenate(idx_list) if idx_list else np.array([], dtype=np.int64)
    return probs, age, idx


def select_pseudo_samples(probs, thr_pos=0.995, thr_neg=0.005, max_n=300):
    probs = np.asarray(probs, dtype=np.float32)
    keep_mask = (probs >= thr_pos) | (probs <= thr_neg)
    keep_idx = np.where(keep_mask)[0]
    if keep_idx.size == 0:
        return keep_idx, np.array([], dtype=np.int64), np.array([], dtype=np.float32)

    conf = np.abs(probs[keep_idx] - 0.5) * 2.0
    order = np.argsort(-conf)
    keep_idx = keep_idx[order][:max_n]
    pseudo_y = (probs[keep_idx] >= 0.5).astype(np.int64)
    pseudo_conf = conf[order][:len(keep_idx)].astype(np.float32)
    return keep_idx, pseudo_y, pseudo_conf
...
def run_one_model(name, train_loader, val_loader, test_loader, device,
                  ...
                  train_ds=None, batch_size=32, num_workers=2, seed=42, pseudo_cfg=None):
    ...
    pseudo_summary = {
        "pseudo_selected_n": 0,
        "pseudo_acc_vs_gt": float("nan"),
        "pseudo_male_n": 0,
        "pseudo_female_n": 0,
    }

    if pseudo_cfg is not None and pseudo_cfg.get("enable", False):
        pseudo_images = pseudo_cfg.get("images", None)
        if pseudo_images is not None and len(pseudo_images) > 0:
            print(f"[{name}] pseudo-label phase started")

            pool_ds = PseudoPoolDataset(
                images=pseudo_images,
                ages=pseudo_cfg.get("ages", None),
            )
            pool_loader = DataLoader(
                pool_ds,
                batch_size=int(pseudo_cfg.get("batch_size", batch_size)),
                shuffle=False,
                num_workers=num_workers,
                pin_memory=True,
            )

            pseudo_probs, pseudo_ages, pseudo_local_idx = predict_probs_unlabeled(
                model, pool_loader, device, use_amp=use_amp
            )

            keep_idx, pseudo_y, pseudo_conf = select_pseudo_samples(
                pseudo_probs,
                thr_pos=float(pseudo_cfg.get("thr_pos", 0.995)),
                thr_neg=float(pseudo_cfg.get("thr_neg", 0.005)),
                max_n=int(pseudo_cfg.get("max_n", 300)),
            )

            pseudo_summary["pseudo_selected_n"] = int(len(keep_idx))
            pseudo_summary["pseudo_male_n"] = int((pseudo_y == 0).sum())
            pseudo_summary["pseudo_female_n"] = int((pseudo_y == 1).sum())

            pseudo_gt = pseudo_cfg.get("sex_gt", None)
            if pseudo_gt is not None and len(keep_idx) > 0:
                gt_i = np.asarray([sex_to_int(s) for s in np.asarray(pseudo_gt)[keep_idx]])
                pseudo_summary["pseudo_acc_vs_gt"] = float((gt_i == pseudo_y).mean())
                print(f"[{name}] pseudo selected={len(keep_idx)} pseudo_acc_vs_gt={pseudo_summary['pseudo_acc_vs_gt']:.3f}")
            else:
                print(f"[{name}] pseudo selected={len(keep_idx)}")

            if len(keep_idx) > 0:
                pseudo_weight = np.full(len(keep_idx), float(pseudo_cfg.get("weight", 0.25)), dtype=np.float32)
...
def run_experiment(exp_name, images, ages, sex, pids, model_names,
    ...
    for mn in model_names:
        row = run_one_model(
            ...
            train_ds=train_ds, batch_size=batch_size, num_workers=num_workers,
            seed=seed, pseudo_cfg=pseudo_cfg
        )
...
def main(argv=None):
    ...
    parser.add_argument("--pseudo_enable", action="store_true",
                        help="enable pseudo-label fine-tuning using external findings pool")
    parser.add_argument("--pseudo_img_npy", type=str, default="",
                        help="pseudo pool npy (dict with keys images/ages/patient_id/sex)")
    parser.add_argument("--pseudo_weight", type=float, default=0.25,
                        help="sample weight for pseudo-labeled samples")
    parser.add_argument("--pseudo_thr_pos", type=float, default=0.995,
                        help="positive threshold for pseudo labeling")
    parser.add_argument("--pseudo_thr_neg", type=float, default=0.005,
    ...
    parser.add_argument("--pseudo_batch", type=int, default=32,
                        help="batch size for pseudo-pool inference")
    parser.add_argument("--pseudo_lr", type=float, default=0.0,
                        help="fine-tuning lr for pseudo phase; 0 uses base_lr*0.5")
    parser.add_argument("--pseudo_lung_mask_npy", type=str, default="",
                        help="optional lung masks for pseudo pool when --do_crop is used")
    ...
    if args.pseudo_enable and args.pseudo_img_npy.strip():
        pdata = np.load(args.pseudo_img_npy, allow_pickle=True).item()
        pseudo_images = np.asarray(pdata["images"]).astype(np.float32)
        pseudo_ages = np.asarray(pdata["ages"])
        pseudo_pids = np.asarray(pdata["patient_id"])
        pseudo_sex_gt = np.asarray(pdata["sex"])
        print("Loaded pseudo pool:", pseudo_images.shape)

        overlap = set(np.asarray(pids).tolist()) & set(np.asarray(pseudo_pids).tolist())
        if len(overlap) > 0:
            keep = np.asarray([pid not in overlap for pid in pseudo_pids], dtype=bool)
            print(f"Pseudo pool overlap with main dataset: {len(overlap)} patients -> removing overlapped samples")
            pseudo_images = pseudo_images[keep]
            pseudo_ages = pseudo_ages[keep]
            pseudo_pids = pseudo_pids[keep]
            pseudo_sex_gt = pseudo_sex_gt[keep]
        print("Pseudo pool after overlap removal:", None if pseudo_images is None else pseudo_images.shape)
    ...
    if args.do_preprocess:
        ...
        if pseudo_images is not None:
            print(">>> preprocessing pseudo pool")
            pseudo_images = preprocess_images(pseudo_images)
    ...
    if args.do_crop:
        ...
        if pseudo_images is not None:
            if args.pseudo_lung_mask_npy:
                pseudo_masks = np.load(args.pseudo_lung_mask_npy, mmap_mode="r")
                if pseudo_masks.ndim == 4:
                    pseudo_masks = pseudo_masks[..., 0]
                pseudo_crop_path = out_dir / "thorax_crop_pseudo_512.npy"
                print(">>> thorax cropping pseudo pool ->", pseudo_crop_path)
                pseudo_images = make_thorax_crop_memmap(pseudo_images, pseudo_masks, pseudo_crop_path, out_size=512)
                print("Cropped pseudo images:", pseudo_images.shape, float(np.min(pseudo_images)), float(np.max(pseudo_images)))
            else:
                print("WARNING: --do_crop was set but --pseudo_lung_mask_npy was not provided. Pseudo pool will NOT be cropped.")
    ...
    if args.pseudo_enable and pseudo_images is not None:
        exp_name += "_PseudoLabel"
